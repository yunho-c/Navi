...
namespace caffe {
	...
	void BasePrefetchingLabelmapDataLayer<Dtype>::LayerSetUp(
		...
		for (int i = 0; i < prefetch_.size(); ++i) {
			...
			if (this->output_edges_) {
				this->prefetch_[i]->labeledge_.mutable_cpu_data();
			}
		}
		...
		if (Caffe::mode() == Caffe::GPU) {
			for (int i = 0; i < prefetch_.size(); ++i) {
				...
				if (this->output_edges_) {
					this->prefetch_[i]->labeledge_.mutable_gpu_data();
				}
			}
		}
		...
		try {
			while (!must_stop()) {
				...
				if (Caffe::mode() == Caffe::GPU) {
					batch->data_.data().get()->async_gpu_push(stream);
					if (this->output_labels_) {
						batch->labelmap_.data().get()->async_gpu_push(stream);
					}
					if (this->output_edges_) {
						batch->labeledge_.data().get()->async_gpu_push(stream);
					}
					CUDA_CHECK(cudaStreamSynchronize(stream));
				}
#endif
	...
	void BasePrefetchingLabelmapDataLayer<Dtype>::Forward_cpu(
		...
		if (this->output_labels_) {
			...
			top[1]->set_cpu_data(prefetch_current_->labelmap_.mutable_cpu_data());
		}
		if (this->output_edges_) {
			// Reshape to loaded edges.
			top[2]->ReshapeLike(prefetch_current_->labeledge_);
			top[2]->set_cpu_data(prefetch_current_->labeledge_.mutable_cpu_data());
		}
	}
